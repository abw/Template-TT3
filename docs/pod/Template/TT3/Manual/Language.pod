...
Template::TT3::Manual::Language - the TT3 template language

=head1 INTRODUCTION

This documentation describes the TT3 template language.
...
which outputs the value of the C<name> variable ('C<[% name %]>').  You can 
create and run this template using the following Perl script:

    use strict;
    use warnings;
    use Template::TT3::Template;
    
    my $template = Template::TT3::Template->new(
        text => 'Hello [% name %]',
    );
...
Note that there's still some work todo on cleaning up the TT3 front-end.
For now, at least, you have to manually create each template as show above,
but it will get nicer.
